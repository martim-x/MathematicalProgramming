...
void task_2_3() {
    ...
    for (int ki = 0; ki < NK; ki++) {
        ...
        for (int r = 0; r < REPEAT; r++) {
            ...
            acc_dp += chrono::duration<double, milli>(t2 - t1).count();
        }
        ...
        if (row.feasible) {
            ...
            for (int r = 0; r < REPEAT; r++) {
                ...
                auto t2 = chrono::high_resolution_clock::now();
                acc_r += chrono::duration<double, milli>(t2 - t1).count();
            }
    ...
    // --- ASCII-график времени ДП ---
    cout << "\n--- График зависимости времени ДП от k ---\n";
    double max_dp = 0;
    for (auto& r : rows) if (r.time_dp > max_dp) max_dp = r.time_dp;
    if (max_dp < 1e-6) max_dp = 1e-6;

    for (auto& row : rows) {
        int bars = max(1, (int)(row.time_dp / max_dp * 45));
        cout << setw(5) << row.kname << " |";
        for (int b = 0; b < bars; b++) cout << "#";
        cout << " " << fixed << setprecision(4)
            << row.time_dp << " мс\n";
    }

    // --- ASCII-график рекурсии (только доступные k) ---
    cout << "\n--- График зависимости времени рекурсии от k ---\n";
    double max_r = 0;
    for (auto& r : rows) if (r.feasible && r.time_r > max_r) max_r = r.time_r;
    if (max_r < 1e-6) max_r = 1e-6;

    for (auto& row : rows) {
        cout << setw(5) << row.kname << " |";
        if (row.feasible) {
            int bars = max(1, (int)(row.time_r / max_r * 45));
            for (int b = 0; b < bars; b++) cout << "#";
            cout << " " << fixed << setprecision(4)
                << row.time_r << " мс";
        }
        else {
            cout << " [N/A — экспоненциальный рост]";
        }
        cout << "\n";
    }
}
...
void task_5() {
    ...
    // ASCII-график для задания 5
    cout << "\n--- График: время рекурсии (#) vs ДП (@) по n ---\n";
    double max_t = 0;
    for (auto v : t_rec_all) if (v > max_t) max_t = v;
    for (auto v : t_dp_all)  if (v > max_t) max_t = v;
    if (max_t < 1e-9) max_t = 1e-9;

    for (int i = 0; i < (int)t_rec_all.size(); i++) {
        int n = i + 2;
        int br = max(1, (int)(t_rec_all[i] / max_t * 40));
        int bd = max(1, (int)(t_dp_all[i] / max_t * 40));
        cout << "n=" << n << " Рек [";
        for (int b = 0; b < br; b++) cout << "#";
        cout << "] " << fixed << setprecision(4) << t_rec_all[i] << " мкс\n";
        cout << "    ДП  [";
        for (int b = 0; b < bd; b++) cout << "@";
        cout << "] " << t_dp_all[i] << " мкс\n";
    }
